...
void CNN(const double flattenedImage[], int imageWidth, int imageHeight, double output[]) {
   ...
   // Max pooling layer
    ...
    // Max pooling layer 2
    double pool2Output[40*40*30];
    maxPooling(conv2Output, pool2Output, imageWidth2 - kernelWidth2 + 1,num_kernels2, pool_size);
    // merge fully connected layer. 
    char a_f2='S';
    double fully1output[2];
    fullyConnectedLayer(pool2Output,fully1output,dense1_weights,dense1_bias,40*40*30,2,a_f2);
    char a_f3='S';
    if(a_f3=='S'){
        softmax(fully1output,2,output);
    }
}
